...
/**
 * The Kanban package root: the durable task plane model (architecture §15.2).
 * The `protocol.ts` surface is pure zod; `state-machine.ts` adds the
 * transition table and expected-version concurrency; `events.ts` wires the
 * model onto Chronicle. The store contract and dispatcher land in P2-2.
 */
