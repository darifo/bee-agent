...
| P3-3 | SecretBroker      | 晚绑定、最小注入、输出脱敏、默认不继承 process.env（白名单基线）                                                                                                                     | P3-1       | M    | secret 不落普通事件/artifact；脱敏测试通过          | doing（SecretBroker 晚绑定接口；无 broker 且请求 secret 时 fail closed；result/output/worldDiff/error 脱敏测试。待实现 Keychain provider、最小环境注入与 artifact/log 扫描）                                                                                  |
| P3-4 | SandboxProvider   | SandboxProvider 契约 + capability report；macOS Seatbelt 与 Linux bwrap 首发实现；平台无法强制的限制 fail closed；fake sandbox 入契约套件                                            | P3-1       | L    | 双平台契约测试通过；secret/network/path escape 测试 | doing（SandboxProvider/capability report/snapshot/diff 契约与 fake suite；InProcessToolSandbox 只接逻辑工具，文件/网络/命令需求 fail closed。待实现 Seatbelt 与 bwrap）                                                                                       |
