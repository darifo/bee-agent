...
| P3-5 | 工具与 Agent 迁入 | calculator/command/python/mcp/remote-agent 全部经 ExecutionWorld；删除全部直接 spawn（§4 静态规则 (b) 生效）；RemoteAgent 完成接线                                                   | P3-1, P3-4 | L    | spawn 禁令 lint 全绿；无允许的直接执行绕行          | doing（静态 scanner + ESLint 已禁止 execution 包外导入 child_process；逻辑工具、`command_run`、`python_run` 已迁移；Python 的 JSON stdin 由 ResourceRequirements 声明并只由平台 provider 注入；待 mcp/remote-agent adapter）                                  |
