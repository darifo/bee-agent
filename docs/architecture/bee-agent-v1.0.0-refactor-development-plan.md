...
> 任务级明细在 Phase 2 验收后细化（§1.3 滚动规则）。

- **WF3-A Capability 管线**：`resolve → validate → authorize → materialize secrets → sandbox → execute → capture diff → verify → emit` 统一管线落地于 execution 包；ActionRequest 声明全集（方案 §13.2）。
- **WF3-B 权限与审批**：交集权限计算 + deny→ask→allow 决策顺序；Approval 变为持久化 Inbox Item + lease，跨重启可恢复；审批展示展开后的真实路径/域名/命令/secret scope。
- **WF3-C SecretBroker**：晚绑定、最小注入、输出脱敏、默认不继承 `process.env`（白名单基线）。
- **WF3-D 沙箱 provider**：SandboxProvider 契约 + capability report；macOS Seatbelt 与 Linux bwrap 首发实现；平台无法强制的限制 fail closed；fake sandbox 入契约套件。
- **WF3-E 工具与 Agent 迁入**：calculator/command/python/mcp/remote-agent 全部经 ExecutionWorld；删除全部直接 spawn（§4 静态规则 (b) 生效）；RemoteAgent 完成接线。
- **WF3-F worktree 与进程树**：Coding bundle 默认 worktree 隔离；取消能终止完整进程树、无孤儿。
- **退出条件**（方案 §19）：无允许的直接执行绕行；低风险动作顺畅、高风险动作展示真实副作用后询问；spawn 禁令 lint 全绿。
